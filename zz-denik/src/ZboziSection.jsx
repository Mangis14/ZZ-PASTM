...
const FIXES = [['â€˘', '•'], ['â€“', '–'], ['â–˛', '▲'], ['â–Ľ', '▼'], ['Â˝', '½'], ['Ă—', '×'], ['Ä›', 'ě'], ['ÄŤ', 'č'], ['ÄŹ', 'ď'], ['Äť', 'ť'], ['Ĺ™', 'ř'], ['Ĺˇ', 'š'], ['Ĺľ', 'ž'], ['ĹĄ', 'ť'], ['Ĺ', 'ň'], ['ĹŻ', 'ů'], ['Ăˇ', 'á'], ['Ă©', 'é'], ['Ă­', 'í'], ['Ăł', 'ó'], ['Ăş', 'ú'], ['Ăš', 'Ú'], ['Ă˝', 'ý']];
const fix = (v) => typeof v === 'string' ? FIXES.reduce((s, [a, b]) => s.split(a).join(b), v) : v;
const norm = (v) => Array.isArray(v) ? v.map(norm) : v && typeof v === 'object' ? Object.fromEntries(Object.entries(v).map(([k, val]) => [fix(k), norm(val)])) : fix(v);
const copper = (price) => !price ? 0 : price.currency === 'gold' ? price.value * 100 : price.currency === 'silver' ? price.value * 10 : price.value;
const money = (n) => n === 0 ? { value: 0, currency: 'copper' } : n >= 100 && n % 100 === 0 ? { value: n / 100, currency: 'gold' } : n >= 10 && n % 10 === 0 ? { value: n / 10, currency: 'silver' } : { value: n, currency: 'copper' };
export const parseWeight = (w) => { const s = fix(String(w || '')).toLowerCase().trim(); if (['–', '-', 'drobné', 'drobný', 'drobná', '', '0'].includes(s)) return 0; if (s.includes('lehk') || s.includes('1/2') || s.includes('½')) return 0.5; if (s.includes('norm')) return 1; if (s.includes('těžk')) return 2; const n = parseFloat(s.replace(',', '.')); return Number.isNaN(n) ? 0 : n; };
const toggle = (arr, value) => arr.includes(value) ? arr.filter((v) => v !== value) : [...arr, value];
const mapItem = (raw) => { const i = norm(raw); return { category: i.Category || '', name: i.Předmět || '', cena: i.Cena || '', price: i.price || null, availability: i.Dostupnost || '', weight: i.Váha || '', materials: i.Suroviny || '', time: i.Čas || '', talent: i.Talent || '', armor: i.Zbroj || '', damage: i.Zranění || '', hands: i.Ruce || '', effect: i.Účinek || '', properties: i.Vlastnosti || '', notes: i.Poznámky || '', bonus: i.Bonus || '', extraEffect: i.Efekt || '' }; };
const materialMatch = (text, category) => { const s = (text || '').toLowerCase(); if (category === 'Železo/Ocel') return s.includes('želez') || s.includes('ocel'); if (category === 'Dřevo') return s.includes('dřev'); if (category === 'Kůže/Useň') return s.includes('useň') || s.includes('usn') || s.includes('kůž'); if (category === 'Látka/Příze') return s.includes('látk') || s.includes('příz'); if (category === 'Zlato') return s.includes('zlat'); if (category === 'Stříbro') return s.includes('stříb'); if (category === 'Byliny') return s.includes('bylin') || s.includes('rostlin'); if (category === 'Kámen/Písek') return s.includes('kám') || s.includes('kamen') || s.includes('písek'); if (category === 'Živočišná (maso/kosti)') return s.includes('maso') || s.includes('tuk') || s.includes('kost') || s.includes('roh'); return false; };

const PriceDisplay = ({ price, cena, size = 'sm' }) => (
    <div className={`flex items-center gap-1 font-mono font-bold ${size === 'lg' ? 'text-base' : 'text-sm'}`}>
        {price ? <><span>{price.value}</span><Circle size={size === 'lg' ? 14 : 10} className={`fill-current ${price.currency === 'gold' ? 'text-[#FFD700]' : price.currency === 'copper' ? 'text-[#9E6649]' : 'text-gray-400'}`} /></> : <span>{fix(cena)}</span>}
    </div>
);
...
const FilterPillGroup = ({ label, icon: Icon, options, selectedValues, onToggle }) => !options.length ? null : (
    <div className="flex flex-col gap-2">
        <div className="flex items-center gap-2 text-[10px] font-bold uppercase tracking-wider text-fl-primary"><Icon size={12} /><span>{label}</span></div>
        <div className="flex gap-2 overflow-x-auto pb-2 scrollbar-hide" onTouchStart={(e) => e.stopPropagation()} onTouchMove={(e) => e.stopPropagation()}>
            {options.map((option) => <button key={option} type="button" onClick={() => onToggle(option)} className={`whitespace-nowrap rounded-full border px-3 py-1.5 text-xs font-bold transition-colors ${selectedValues.includes(option) ? 'border-fl-primary bg-fl-primary text-white' : 'border-fl-primary bg-transparent text-fl-primary hover:bg-fl-primary/10'}`}>{option}</button>)}
        </div>
    </div>
);
...
const ZboziSection = ({ addItemToInventory }) => {
    ...
    const talents = useMemo(() => [...new Set(current.map((i) => i.talent).filter(Boolean).filter((v) => v !== '–'))].sort(), [current]);
    const materials = useMemo(() => { const set = new Set(); current.forEach((i) => { const s = (i.materials || '').toLowerCase(); if (!s || s === '–') return; if (s.includes('želez') || s.includes('ocel')) set.add('Železo/Ocel'); if (s.includes('dřev')) set.add('Dřevo'); if (s.includes('useň') || s.includes('usn') || s.includes('kůž')) set.add('Kůže/Useň'); if (s.includes('látk') || s.includes('příz')) set.add('Látka/Příze'); if (s.includes('zlat')) set.add('Zlato'); if (s.includes('stříb')) set.add('Stříbro'); if (s.includes('bylin') || s.includes('rostlin')) set.add('Byliny'); if (s.includes('kám') || s.includes('kamen') || s.includes('písek')) set.add('Kámen/Písek'); if (s.includes('maso') || s.includes('tuk') || s.includes('kost') || s.includes('roh')) set.add('Živočišná (maso/kosti)'); }); return [...set].sort(); }, [current]);
    const weights = useMemo(() => [...new Set(current.map((i) => i.weight).filter(Boolean).filter((v) => v !== '–' && v !== '-'))].sort((a, b) => parseWeight(a) - parseWeight(b)), [current]);
    const rarities = useMemo(() => [...new Set(current.map((i) => i.availability).filter(Boolean).filter((v) => v !== '–' && v !== '-'))].sort(), [current]);
    const armorValues = useMemo(() => [...new Set(current.map((i) => i.armor).filter(Boolean).filter((v) => v !== '–' && v !== '-'))].sort((a, b) => parseInt(a, 10) - parseInt(b, 10)), [current]);
    const damageValues = useMemo(() => [...new Set(current.map((i) => i.damage).filter(Boolean).filter((v) => v !== '–' && v !== '-'))].sort((a, b) => parseInt(a, 10) - parseInt(b, 10)), [current]);
    const handsValues = useMemo(() => [...new Set(current.map((i) => i.hands).filter(Boolean).filter((v) => v !== '–' && v !== '-'))].sort(), [current]);
    const timeValues = useMemo(() => [...new Set(current.map((i) => i.time).filter(Boolean).filter((v) => v !== '–' && v !== '-'))].sort(), [current]);
    const activeFilterCount = talentFilter.length + materialFilter.length + weightFilter.length + rarityFilter.length + armorFilter.length + damageFilter.length + timeFilter.length + handsFilter.length;
    const resetFilters = () => { setTalentFilter([]); setMaterialFilter([]); setWeightFilter([]); setRarityFilter([]); setArmorFilter([]); setDamageFilter([]); setTimeFilter([]); setHandsFilter([]); };
    const filtered = useMemo(() => [...current].filter((item) => { if (!item.name || item.name === 'Předmět') return false; const q = search.toLowerCase(); const searchOk = item.name.toLowerCase().includes(q) || item.effect.toLowerCase().includes(q) || item.properties.toLowerCase().includes(q) || item.notes.toLowerCase().includes(q); const talentOk = !talentFilter.length || talentFilter.some((v) => item.talent.includes(v)); const materialOk = !materialFilter.length || materialFilter.some((v) => materialMatch(item.materials, v)); const weightOk = !weightFilter.length || weightFilter.includes(item.weight); const rarityOk = !rarityFilter.length || rarityFilter.includes(item.availability); const armorOk = !armorFilter.length || armorFilter.includes(item.armor); const damageOk = !damageFilter.length || damageFilter.includes(item.damage); const timeOk = !timeFilter.length || timeFilter.includes(item.time); const handsOk = !handsFilter.length || handsFilter.includes(item.hands); return searchOk && talentOk && materialOk && weightOk && rarityOk && armorOk && damageOk && timeOk && handsOk; }).sort((a, b) => sortOrder === 'asc' ? copper(a.price) - copper(b.price) : copper(b.price) - copper(a.price)), [current, search, sortOrder, talentFilter, materialFilter, weightFilter, rarityFilter, armorFilter, damageFilter, timeFilter, handsFilter]);
    const filterGroups = [{ label: 'Talent', icon: Star, options: talents, selectedValues: talentFilter, setter: setTalentFilter }, { label: 'Suroviny', icon: Hammer, options: materials, selectedValues: materialFilter, setter: setMaterialFilter }, { label: 'Váha', icon: Shield, options: weights, selectedValues: weightFilter, setter: setWeightFilter }, { label: 'Dostupnost', icon: Clock, options: rarities, selectedValues: rarityFilter, setter: setRarityFilter }, { label: 'Zbroj', icon: Shield, options: armorValues, selectedValues: armorFilter, setter: setArmorFilter }, { label: 'Zranění', icon: Sword, options: damageValues, selectedValues: damageFilter, setter: setDamageFilter }, { label: 'Výroba (čas)', icon: Clock, options: timeValues, selectedValues: timeFilter, setter: setTimeFilter }, { label: 'Ruce', icon: HandCoins, options: handsValues, selectedValues: handsFilter, setter: setHandsFilter }].filter((g) => g.options.length);
    const addToCart = (item) => setCart((prev) => { const idx = prev.findIndex((i) => i.name === item.name); if (idx >= 0) { const next = [...prev]; next[idx] = { ...next[idx], qty: next[idx].qty + 1 }; return next; } return [...prev, { ...item, qty: 1 }]; });
    const removeFromCart = (index) => setCart((prev) => { const next = [...prev]; next[index].qty > 1 ? next[index] = { ...next[index], qty: next[index].qty - 1 } : next.splice(index, 1); return next; });
    const incrementCart = (index) => setCart((prev) => { const next = [...prev]; next[index] = { ...next[index], qty: next[index].qty + 1 }; return next; });
    const completelyRemoveCart = (index) => setCart((prev) => { const next = [...prev]; next.splice(index, 1); return next; });
    const clearCart = () => setCart([]);

    return (
        <section id="zbozi-section" className="animate-in fade-in slide-in-from-bottom-4 duration-500 scroll-mt-20">
            <SectionHeader title="Zboží a služby" icon={ShoppingBag} />
            <div className="mb-6 flex flex-wrap gap-2">{tabs.map((tab) => <button key={tab.id} onClick={() => { setActiveTab(tab.id); resetFilters(); }} className={`flex items-center gap-2 rounded-sm px-3 py-2 text-xs font-bold uppercase tracking-wider transition-all ${activeTab === tab.id ? 'scale-105 bg-fl-primary text-white shadow-md' : 'bg-fl-paper text-fl-text-muted hover:bg-fl-border hover:text-fl-surface'}`}><tab.icon size={14} />{tab.id}</button>)}</div>
            <Card className="mb-6">
                <div className="flex flex-col gap-3">
                    <div className="flex h-10 gap-2">
                        <div className="relative flex-1"><Search className="absolute left-3 top-1/2 -translate-y-1/2 text-fl-primary" size={16} /><input type="text" placeholder="Hledat předmět, účinek..." className="h-full w-full rounded-sm border border-fl-border bg-fl-paper-bright pl-9 pr-3 text-sm text-fl-surface placeholder:text-fl-border focus:border-fl-primary focus:outline-none" value={search} onChange={(e) => setSearch(e.target.value)} /></div>
                        <button onClick={() => setSortOrder((p) => p === 'asc' ? 'desc' : 'asc')} className="flex min-w-[70px] items-center justify-center gap-1 rounded-sm border border-fl-border px-3 text-xs font-bold uppercase tracking-wider text-fl-surface-hover hover:bg-fl-border hover:text-fl-surface" title="Řadit podle ceny"><ArrowUpDown size={14} /><span className="hidden sm:inline">Cena</span> {sortOrder === 'asc' ? '▲' : '▼'}</button>
                        <button onClick={() => setShowFilters((p) => !p)} className={`relative flex items-center justify-center rounded-sm border px-3 shadow-sm ${showFilters || activeFilterCount ? 'border-fl-primary bg-fl-primary text-white' : 'border-fl-border bg-fl-paper text-fl-surface hover:bg-fl-border'}`} title="Podrobné filtrování"><Filter size={16} />{!!activeFilterCount && <span className="absolute -right-1.5 -top-1.5 flex h-4 min-w-4 items-center justify-center rounded-full border border-white bg-red-600 px-1 text-[9px] font-bold leading-none text-white">{activeFilterCount}</span>}</button>
                    </div>
