...
import { Anchor, Brain, CheckCircle, Smile, Sword } from 'lucide-react';
...
const AttributeBox = ({ label, value, onChange, onRoll, icon: Icon }) => (
    <div className="border border-fl-primary/30 bg-fl-paper-bright rounded-sm group hover:border-fl-primary/60 transition-colors overflow-hidden">
        <div
            className="flex justify-between items-center px-2 pt-2 pb-1 cursor-pointer hover:text-fl-primary"
            onClick={() => onRoll && onRoll(value.current, 0, 0)}
            title={`Hodit na atribut ${label}`}
        >
            <span className="font-bold uppercase text-[11px] tracking-wide text-fl-text-muted group-hover:text-fl-primary transition-colors">
                {label}
            </span>
    ...
    <>
        ...
        <Card className="bg-fl-paper/30 mt-3">
            ...
            <div className="grid grid-cols-4 gap-2">
                {[
                    ...
                    <label key={condition.id} className="flex flex-col items-center cursor-pointer group">
                        <div className={`w-7 h-7 border-2 rounded-sm flex items-center justify-center mb-1 transition-colors ${
                            char.conditions?.[condition.id]
                                ? 'border-red-700 bg-red-700 text-white'
                                : 'border-fl-primary bg-fl-card'
                        }`}>
                            {char.conditions?.[condition.id] && <CheckCircle size={16} />}
                        </div>
                        <input
                            type="checkbox"
                            className="hidden"
                            checked={!!char.conditions?.[condition.id]}
                            onChange={(event) => updateField(`conditions.${condition.id}`, event.target.checked)}
                        />
                        <span className={`text-[9px] font-bold uppercase text-center leading-tight ${
                            char.conditions?.[condition.id] ? 'text-red-700 dark:text-red-400' : 'text-fl-text-muted'
                        }`}>
                            {condition.label}
                        </span>
                    </label>
        ...
        <section className="grid grid-cols-2 gap-3 mt-3">
            {[
                ...
                <div key={field} className="border-2 border-fl-border bg-fl-card rounded-sm shadow-sm text-center px-2 py-3">
